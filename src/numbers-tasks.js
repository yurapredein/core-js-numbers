...
function getAverage(value1, value2) {
  ...
  if (sum === Infinity) return 1.7976931348623157e308;
  return sum / 2;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const form = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(average / form);
}
...
function getSumOfDigits(num) {
  let array = [];
  array = Array.from(num.toString(), Number);
  let sum = 0;
